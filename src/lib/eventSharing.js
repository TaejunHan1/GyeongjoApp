...
export async function getSharedEventsForCurrentUser(user = null) {
  const currentUser = await resolveUser(user);
  if (!currentUser?.id)
    return { success: false, data: [], error: "not_logged_in" };
...
export async function inviteEventMember({
  ...
  if (!result?.success)
    return { success: false, error: result?.error || "invite_failed" };
...
export async function removeEventMember(memberId, userId = null) {
  ...
  const result = Array.isArray(data) ? data[0] : data;
  if (!result?.success)
    return { success: false, error: result?.error || "remove_failed" };
  return { success: true };
}

export async function updateSharedEventAdditionalInfo(
  eventId,
  additionalInfo,
  userId = null,
) {
