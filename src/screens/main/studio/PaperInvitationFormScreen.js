...
export default function PaperInvitationFormScreen({ navigation, route }) {
  ...
  const frontData = invitation?.layout?.frontData || {};
  const backData = invitation?.layout?.backData || {};
  const [formStep, setFormStep] = useState(1);
  const [sampleOpen, setSampleOpen] = useState(false);
  const [frontGreetingText, setFrontGreetingText] = useState(
  ...
  const [showGroomFather, setShowGroomFather] = useState(
    backData.showGroomFather !== false,
  );
  const [showGroomMother, setShowGroomMother] = useState(
    backData.showGroomMother !== false,
  );
  const [showBrideFather, setShowBrideFather] = useState(
    backData.showBrideFather !== false,
  );
  const [showBrideMother, setShowBrideMother] = useState(
  ...
  const supportsFrontGreeting = !!template?.text?.greeting;
  ...
  const fillTestData = () => {
    ...
    setBrideFather("이동훈");
    setBrideMother("최은영");
    setShowGroomFather(true);
    setShowGroomMother(true);
    setShowBrideFather(true);
    setShowBrideMother(true);
    setGroomRelation("장남");
    setBrideRelation("장녀");
  };
  ...
  const handleNext = () => {
    ...
    navigation.navigate("PaperInvitationLayout", {
      ...
      formData: {
        ...
        venue,
        frontData: supportsFrontGreeting
          ? {
              greetingText: frontGreetingText.trim(),
            }
          : null,
  ...
  if (!template) {
    return (
      <SafeAreaView style={s.root}>
        <Text style={{ padding: 20, color: TC.inkMuted }}>
          템플릿 정보가 없습니다
        </Text>
      </SafeAreaView>
    );
  ...
  const renderParentInput = ({
    ...
    <View
      style={[s.inputCellInRow, { flex: 1 }, !visible && s.parentInputDisabled]}
    >
      <View style={s.parentLabelRow}>
        <Text style={[s.inputLabel, !visible && s.inputLabelMuted]}>
          {label}
        </Text>
        <TouchableOpacity
          style={[
            s.parentToggle,
            visible ? s.parentToggleOn : s.parentToggleOff,
          ]}
        ...
        >
          <Text
            style={[
              s.parentToggleText,
              visible ? s.parentToggleTextOn : s.parentToggleTextOff,
            ]}
          >
            {visible ? "표시" : "숨김"}
          </Text>
      ...
      <TextInput
        ...
        placeholder={visible ? "성함" : "표시하지 않음"}
        placeholderTextColor={TC.inkDim}
        style={[s.input, !visible && s.inputDisabledText]}
        maxLength={12}
        editable={visible}
      />
    </View>
        ...
        {supportsBackSide && (
          <>
            <View style={[s.stepDot, formStep >= 2 && s.stepDotActive]}>
              <Text
                style={[s.stepDotText, formStep < 2 && { color: TC.inkMuted }]}
              >
                2
              </Text>
            </View>
      ...
      >
        ...
        >
          ...
          <View style={s.testFillWrap}>
            ...
            <Text style={s.testFillHint}>
              앞면·뒷면 정보와 기본 사진을 한 번에 채웁니다.
            </Text>
          </View>
          ...
          {!supportsBackSide || formStep === 1 ? (
            <>
              ...
              <View style={s.tipCard}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={s.tipText}>
                    템플릿 분위기에 맞춰 "김 민 수"처럼 한 칸씩 띄어 입력해도
                    좋아요.
                  </Text>
                </View>
              </View>
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  {dateObj ? (
                    ...
                    <>
                      <Text style={s.dateTimeMainPlaceholder}>
                        날짜·시간 선택
                      </Text>
                      <Text style={s.dateTimeSub}>
                        예식 날짜와 시간을 골라주세요
                      </Text>
                    </>
                  )}
              ...
              <Text style={s.sectionLabel}>
                예식장{" "}
                <Text style={{ color: TC.inkDim, fontWeight: "500" }}>
                  (선택)
                </Text>
              </Text>
              ...
              {supportsFrontGreeting && (
                <>
                  <Text style={s.sectionLabel}>하단 문구</Text>
                  <View style={s.inputCell}>
                    <Text style={s.inputLabel}>문구</Text>
                    <TextInput
                      value={frontGreetingText}
                      onChangeText={setFrontGreetingText}
                      placeholder="하단 문구를 입력해주세요"
                      placeholderTextColor={TC.inkDim}
                      style={[s.input, s.compactTextArea]}
                      multiline
                      blurOnSubmit={false}
            ...
            <>
              ...
              <View style={s.sampleCard}>
                <TouchableOpacity
                  style={s.sampleHeader}
                  onPress={() => setSampleOpen((prev) => !prev)}
                  activeOpacity={0.75}
                >
                  <View>
                    <Text style={s.sampleHeaderTitle}>샘플 문구</Text>
                    <Text style={s.sampleHeaderSub}>
                      짧은/중간/긴 문구 중 선택
                    </Text>
                  </View>
                  <Ionicons
                    name={sampleOpen ? "chevron-up" : "chevron-down"}
                    size={18}
                    color={TC.inkMuted}
                  />
                </TouchableOpacity>
                    ...
                    {[
                      {
                        id: "short",
                        title: "짧은 문구",
                        items: INVITATION_TEXT_SAMPLES.short,
                      },
                      {
                        id: "medium",
                        title: "중간 문구",
                        items: INVITATION_TEXT_SAMPLES.medium,
                      },
                      {
                        id: "long",
                        title: "긴 문구",
                        items: INVITATION_TEXT_SAMPLES.long,
                      },
                    ].map((group) => (
                      <View key={group.id} style={s.sampleGroup}>
                        ...
                        <ScrollView
                          ...
                          contentContainerStyle={s.sampleRow}
                        >
              ...
              <View style={s.tipCard}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={s.tipText}>
                    "김 성 호"처럼 띄어 쓰면 더 격식 있는 느낌으로 연출할 수
                    있어요.
                  </Text>
                </View>
              </View>
              <View style={s.row}>
                {renderParentInput({
                  ...
                  visible: showGroomFather,
                  onToggle: setShowGroomFather,
                })}
                {renderParentInput({
                  label: "신랑 어머님",
                  value: groomMother,
                  onChangeText: setGroomMother,
                  visible: showGroomMother,
                  onToggle: setShowGroomMother,
                })}
              </View>
              <View style={s.row}>
                {renderParentInput({
                  ...
                  visible: showBrideFather,
                  onToggle: setShowBrideFather,
                })}
                {renderParentInput({
                  label: "신부 어머님",
                  value: brideMother,
                  onChangeText: setBrideMother,
                  visible: showBrideMother,
                  onToggle: setShowBrideMother,
                })}
              </View>
                    ...
                    return (
                      ...
                      >
                        <Text
                          style={[
                            s.relationChipText,
                            active && s.relationChipTextActive,
                          ]}
                        >
                          {option}
                        </Text>
              ...
              <View style={s.backHintCard}>
                ...
                <Text style={s.backHintText}>
                  달력은 1단계에서 선택한 예식 날짜를 기준으로 자동 생성돼요.
                  다음 단계에서 달력 스타일도 바꿀 수 있어요.
                </Text>
              </View>
            </>
          )}
        ...
        <View style={s.bottomBar}>
          ...
          <TouchableOpacity
            style={[
              s.nextBtn,
              supportsBackSide && formStep === 2 && { flex: 1 },
            ]}
            onPress={handleNext}
            activeOpacity={0.85}
          >
