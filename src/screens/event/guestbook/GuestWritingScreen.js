...
const GUEST_WRITING_DEBUG = false;
const HANDWRITING_CAPTURE_FORMAT = "jpg";
const HANDWRITING_CAPTURE_QUALITY = 0.72;
const HANDWRITING_CAPTURE_MIME = "image/jpeg";
const HANDWRITING_CAPTURE_SCALE = 0.65;
...
const countInkPoints = (strokes = []) =>
  strokes.reduce(
    (sum, stroke) => sum + (Array.isArray(stroke) ? stroke.length : 0),
    0,
  );
...
const logGuestWritingTiming = (label, payload = {}) => {
  if (!GUEST_WRITING_DEBUG) return;
  console.log(`[GuestWriting][timing] ${label}`, payload);
};

const prepareDigitalInkRecognizer = async () => {
  if (!DigitalInk) return false;
  const prepareRecognizer =
    DigitalInk.warmUpRecognizer || DigitalInk.downloadModel;
  if (!prepareRecognizer) return false;
  return prepareRecognizer(DIGITAL_INK_LANGUAGE);
};

const DEFAULT_OWNED_PAPER_IDS = PAPER_TEMPLATES.filter(
  (template) => Number(template.price || 0) <= 0,
...
const PaperBrandMark = ({ template, preview = false }) => (
  ...
  >
    <Image
      ...
      resizeMode="contain"
    />
    <Text style={preview ? intro.brandTaglinePreview : draw.brandTagline}>
      정성을 담아서
    </Text>
  </View>
);
...
const PaperLivePreview = ({ template, width, height = 154 }) => {
  ...
  const content = (
    <>
      {!template.image &&
        renderPaperPattern(template, width - 20, previewHeight, true)}
      <View style={intro.livePreviewShade} />
      <PaperBrandMark template={template} preview />
      <Text style={[intro.livePreviewName, { color: template.ink }]}>
        홍길동
      </Text>
    </>
  );
  ...
  if (template.image) {
    return (
      <ImageBackground
        ...
        style={[
          intro.livePreview,
          { height: previewHeight, backgroundColor: template.bg },
        ]}
...
export default function GuestWritingScreen({ navigation, route }) {
  ...
  const initialPaperId = PAPER_TEMPLATES.some(
    (template) => template.id === paperTemplateId,
  )
    ? paperTemplateId
  ...
  const ownedPaperIdSet = useMemo(
    () => new Set([...DEFAULT_OWNED_PAPER_IDS, ...ownedPaperIds]),
    [ownedPaperIds],
  );
  const selectedPaperOwned =
    Number(selectedPaper.price || 0) <= 0 ||
    ownedPaperIdSet.has(selectedPaper.id);
  const ownedPaperTemplates = useMemo(
    () =>
      PAPER_TEMPLATES.filter(
        (template) =>
          Number(template.price || 0) <= 0 || ownedPaperIdSet.has(template.id),
      ),
    [ownedPaperIdSet],
  );
  ...
  const hintAnims = useRef(
    HINT_STROKES.map(() => new Animated.Value(0)),
  ).current;
  ...
  const enterDrawing = async () => {
    ...
    // Digital Ink 모델을 미리 준비해서 완료 시 인식 실패/대기 시간을 줄인다.
    if (DigitalInk) {
      const warmupStartedAt = nowMs();
      logGuestWritingTiming("recognizer_warmup_start", {
        platform: Platform.OS,
        language: DIGITAL_INK_LANGUAGE,
        method: DigitalInk.warmUpRecognizer
          ? "warmUpRecognizer"
          : "downloadModel",
      });
      prepareDigitalInkRecognizer()
        .then((ready) => {
          logGuestWritingTiming("recognizer_warmup_done", {
            warmupMs: Math.round(nowMs() - warmupStartedAt),
            ready,
          });
        })
        .catch((error) => {
          logGuestWritingTiming("recognizer_warmup_failed", {
            warmupMs: Math.round(nowMs() - warmupStartedAt),
  ...
  const handlePurchaseSelected = useCallback(async () => {
    ...
    try {
      ...
      if (result?.success) {
        setOwnedPaperIds((prev) =>
          Array.from(
            new Set([...DEFAULT_OWNED_PAPER_IDS, ...prev, selectedPaper.id]),
          ),
        );
        setCreditBalance(result.balance ?? 0);
        Alert.alert(
  ...
  const handleUnlockAndStart = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert("오류", "방명록 이용권 처리 중 오류가 발생했습니다.", [
        { text: "확인" },
      ]);
    } finally {
      setPurchaseLoading(false);
    ...
    PanResponder.create({
      ...
      onPanResponderGrant: (evt) => {
        ...
        if (stylusTouch) {
          ...
          inkPointsRef.current = [
            {
              x: stylusTouch.locationX,
              y: stylusTouch.locationY,
              t: Date.now(),
            },
          ];
          setCurrentPath(
        ...
        if (touches.length === 1) {
          ...
          pendingTouches.current.clear();
          inkPointsRef.current = [
            { x: t.locationX, y: t.locationY, t: Date.now() },
          ];
          setCurrentPath(
            `M${t.locationX.toFixed(1)},${t.locationY.toFixed(1)}`,
          );
          return;
        }
      ...
      onPanResponderMove: (evt) => {
        ...
        if (drawingTouchId.current !== null) {
          ...
          setCurrentPath(
            (prev) =>
              `${prev} L${target.locationX.toFixed(1)},${target.locationY.toFixed(1)}`,
          );
          return;
        }
  ...
  const handleDone = async () => {
    ...
    try {
      ...
      const strokesSnapshot = inkStrokesRef.current.map((stroke) => [
        ...stroke,
      ]);
      const pointCount = countInkPoints(strokesSnapshot);
      const captureWidth = Math.max(
        1,
        Math.round(screenSize.width * HANDWRITING_CAPTURE_SCALE),
      );
      const captureHeight = Math.max(
        1,
        Math.round(screenSize.height * HANDWRITING_CAPTURE_SCALE),
      );
      logGuestWritingTiming("done_pressed", {
        platform: Platform.OS,
        eventId: event?.id,
        side,
        ...
        captureFormat: HANDWRITING_CAPTURE_FORMAT,
        captureQuality: HANDWRITING_CAPTURE_QUALITY,
        captureScale: HANDWRITING_CAPTURE_SCALE,
        captureWidth,
        captureHeight,
        digitalInkLoaded: !!DigitalInk,
      });
      const createRecognitionDebug = () => ({
        ...
        capture: {
          format: HANDWRITING_CAPTURE_FORMAT,
          quality: HANDWRITING_CAPTURE_QUALITY,
          scale: HANDWRITING_CAPTURE_SCALE,
          width: captureWidth,
          height: captureHeight,
          mimeType: HANDWRITING_CAPTURE_MIME,
        },
      });
      ...
      const recognizeHandwriting = async () => {
        ...
        logGuestWritingTiming("recognize_start", {
          strokeCount: strokesSnapshot.length,
          pointCount,
          moduleLoaded: !!DigitalInk,
        });
        ...
        if (DigitalInk) {
          try {
            if (strokesSnapshot.length > 0) {
              ...
              logGuestWritingTiming("recognize_prepare_start", {
                platform: Platform.OS,
                language: DIGITAL_INK_LANGUAGE,
              });
              const modelReady = await prepareDigitalInkRecognizer();
              recognitionDebug.modelReadyBeforeRecognize = modelReady;
              recognitionDebug.timing = {
                ...(recognitionDebug.timing || {}),
                prepareMs: Math.round(nowMs() - prepareStartedAt),
              };
              logGuestWritingTiming("recognize_prepare_done", {
                prepareMs: recognitionDebug.timing.prepareMs,
                modelReady,
              });

              if (!modelReady) {
                recognitionDebug.stage = "MODEL_PREPARE_FAILED";
                recognitionDebug.error =
                  "Digital Ink model is not ready after prepare.";
                return { inkCandidates, recognitionDebug };
              }

              inkCandidates = await DigitalInk.recognize(
                strokesSnapshot,
                DIGITAL_INK_LANGUAGE,
              );
              recognitionDebug.stage = "RECOGNIZE";
              recognitionDebug.rawCandidates = inkCandidates;
              if (
                GUEST_WRITING_DEBUG &&
                inkCandidates.length === 0 &&
                DigitalInk.recognizeWithDebug
              ) {
                const debugResult = await DigitalInk.recognizeWithDebug(
                  strokesSnapshot,
                  DIGITAL_INK_LANGUAGE,
                );
                recognitionDebug = {
        ...
        logGuestWritingTiming("recognize_done", {
          recognizeMs: recognitionDebug.timing.recognizeMs,
          stage: recognitionDebug.stage,
          rawCandidates: recognitionDebug.rawCandidates,
          normalizedCandidates: recognitionDebug.normalizedCandidates,
          error: recognitionDebug.error,
        });
        return { inkCandidates, recognitionDebug };
      };
      ...
      const captureHandwriting = async () => {
        const startedAt = nowMs();
        logGuestWritingTiming("capture_start", {
          format: HANDWRITING_CAPTURE_FORMAT,
          quality: HANDWRITING_CAPTURE_QUALITY,
          screenWidth: screenSize.width,
          screenHeight: screenSize.height,
        ...
        logGuestWritingTiming("capture_done", {
          captureMs: result.captureMs,
          uriLength: String(uri || "").length,
      ...
      logGuestWritingTiming("navigate_guest_confirm", {
        totalBeforeNavigateMs: timing.totalBeforeNavigateMs,
        recognizeMs: timing.recognizeMs,
      ...
      navigation.navigate("GuestConfirm", {
        ...
        handwritingMimeType: uri.mimeType,
        handwritingCaptureFormat: uri.format,
        side,
        inkCandidates,
        recognitionDebug: GUEST_WRITING_DEBUG
          ? {
              ...recognitionDebug,
              timing,
            }
      ...
      logGuestWritingTiming("done_failed", {
      ...
      <View style={intro.container}>
        ...
        <View style={intro.body}>
          <View style={intro.eventBlock}>
            <View
              style={[intro.sidePill, { backgroundColor: sideColor + "14" }]}
            >
              <Text style={[intro.sidePillText, { color: sideColor }]}>
                {sideCode}
              </Text>
              <Text style={[intro.sideName, { color: sideColor }]}>
                {sideLabel}
              </Text>
            </View>
          ...
          <View style={intro.sectionHeader}>
            <View>
              ...
              <Text style={intro.sectionSub}>
                {isFuneralEvent
                  ? "조문 접수 화면에 표시될 종이 템플릿"
                  : "축의대 화면에 표시될 종이 템플릿"}
              </Text>
            </View>
            <View style={intro.creditPill}>
              <Text style={intro.creditPillText}>
                {creditBalance == null
                  ? "크레딧 -"
                  : `잔액 ${Number(creditBalance).toLocaleString("ko-KR")}크레딧`}
              </Text>
            </View>
          </View>
          ...
          <ScrollView
            horizontal
            style={[
              intro.templateScroller,
              isCompactIntro && intro.templateScrollerCompact,
            ]}
          ...
          >
            {PAPER_TEMPLATES.map((template) => {
              ...
              const owned =
                ...
                ownedPaperIdSet.has(template.id);
              return (
                <TouchableOpacity
                  key={template.id}
                  style={[
                ...
                >
                  <PaperLivePreview
                    template={template}
                    width={carouselCardWidth - 20}
                    height={previewHeight}
                  />
                  <View
                    style={[
                      intro.templateInfoRow,
                      isCompactIntro && intro.templateInfoRowCompact,
                    ]}
                  >
                    <View style={intro.templateMeta}>
                      <Text style={intro.templateName} numberOfLines={1}>
                        {template.name}
                      </Text>
                      <Text style={intro.templateSub} numberOfLines={1}>
                        {template.subtitle}
                      </Text>
                    </View>
                    <View
          ...
          <View
            style={[
              intro.selectedCard,
              isCompactIntro && intro.selectedCardCompact,
            ]}
          >
          ...
          <View
            style={[
              intro.ownedSection,
              isCompactIntro && intro.ownedSectionCompact,
            ]}
          >
            <TouchableOpacity
              style={intro.ownedHeader}
              onPress={() =>
                isCompactIntro && setOwnedSectionExpanded((prev) => !prev)
              }
              activeOpacity={isCompactIntro ? 0.78 : 1}
            >
              <Text style={intro.ownedTitle}>보유한 배경</Text>
              <View style={intro.ownedHeaderRight}>
                <Text style={intro.ownedCount}>
                  {storeLoading ? "확인 중" : `${ownedPaperTemplates.length}개`}
                </Text>
                {isCompactIntro && (
                  <Ionicons
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={intro.ownedChipTextWrap}>
                        <Text style={intro.ownedChipName} numberOfLines={1}>
                          {template.name}
                        </Text>
                        <Text style={intro.ownedChipSub}>
                          {Number(template.price || 0) <= 0
                            ? "기본"
                            : "구매 완료"}
                        </Text>
        ...
        <View style={intro.footer}>
          {!storeLoading && (
            <View style={intro.accessNotice}>
              <View style={intro.accessNoticeIcon}>
                <Ionicons
                  name={
                    eventAccessUnlocked ? "checkmark-circle" : "ticket-outline"
                  }
                  size={17}
          ...
          <TouchableOpacity
            ...
            style={[
              ...
              {
                backgroundColor:
                  selectedPaperOwned && eventAccessUnlocked
                    ? sideColor
                    : "#191F28",
              },
  ...
  return (
    <View style={{ flex: 1, backgroundColor: selectedPaper.bg }}>
      ...
      {!hasStrokes && (
        ...
        >
          <Svg style={StyleSheet.absoluteFill} width={SW} height={SH}>
            ...
            */}
            <G
              transform={`translate(${SW / 2 - 312}, ${SH / 2 - 155}) scale(2.5)`}
            >
              {HINT_STROKES.map((stroke, i) => {
                const dashOffset = hintAnims[i].interpolate({
                  inputRange: [0, 1],
                  outputRange: [200, 0],
                });
      ...
      <Animated.View
        style={[draw.controls, { opacity: controlsOpacity }]}
        ...
        <View style={draw.rightBtns}>
          ...
          <TouchableOpacity
            style={[
              ...
              (!hasStrokes || isProcessing) && draw.doneBtnDisabled,
            ]}
            onPress={handleDone}
            disabled={!hasStrokes || isProcessing}
          >
            <Text style={draw.doneBtnText}>
